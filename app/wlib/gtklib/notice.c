		...
		const char * alternate )
{
  ...
  gtk_box_pack_start (GTK_BOX (vbox), hbox1, FALSE, TRUE, 0);
	...
	g_signal_connect( GTK_OBJECT(nw->butt[0]), "clicked", G_CALLBACK(doNotice), (void*)1 );
	...
	if( cancel ) {
	  ...
	  gtk_container_set_border_width (GTK_CONTAINER (nw->butt[ 1 ]), 3);
		g_signal_connect( GTK_OBJECT(nw->butt[1]), "clicked", G_CALLBACK(doNotice), (void*)0 );
	  gtk_widget_set_can_default(nw->butt[ 1 ], TRUE );
		...
		if( alternate ) {
			...
			g_signal_connect( GTK_OBJECT(nw->butt[2]), "clicked", G_CALLBACK(doNotice), (void*)-1 );
